...
def OnD20PythonActionPerformLearn(attachee, args, evt_obj):
	...
	try:
		print('OnD20PythonActionPerformLearn')
		print('data1: {}, arg2: {}'.format(evt_obj.d20a.data1, args.get_arg(2)))

		inv_idx = evt_obj.d20a.data1
		if args.get_arg(2) != inv_idx:
			return 0
		item = attachee.inventory_item(inv_idx)
		if (not item):
			print("not item for attachee: {}, inv_idx: {}".format(attachee, inv_idx))
			debug.breakp("no knowstone item!")
		if (item):
